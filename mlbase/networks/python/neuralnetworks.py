...
class NeuralNetwork:
    ...
    def __init__(self, n_inputs, n_hiddens_list, n_outputs, activation="tanh"):
        ...
        for layeri in range(self.n_hidden_layers):
            ...
            ni = n_in_layer
        self.W = (
            1 / np.sqrt(1 + ni) * np.random.uniform(-1, 1, size=(1 + ni, n_outputs))
        )

        if activation == "relu":
            self.activation = lambda weighted_sum: weighted_sum * (weighted_sum > 0)
            self.d_activation = lambda activation_val: 1.0 * (activation_val > 0)

        elif activation == "logistic":
            self.activation = lambda weighted_sum: 1.0 / (1.0 + np.exp(-weighted_sum))
    ...
    def _unpack(self, w):
        ...
        for i in range(self.n_hidden_layers):
            self.Vs[i][:] = w[
                first : first + (1 + n_this_layer) * self.n_hiddens_list[i]
            ].reshape((1 + n_this_layer, self.n_hiddens_list[i]))
...
if __name__ == "__main__":
    ...
    nnet = NeuralNetwork(X.shape[1], [5] * 2, T.shape[1])
    nnet.train(X, T, n_epochs, method="sgd", learning_rate=0.01, momentum_rate=0.5)
    Y = nnet.use(X)
    print(
