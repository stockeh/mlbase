        ...
        if success:
            ...
            if kappa < float_precision:
                return {
                    ...
                    "n_iterations": iteration,
                    "wtrace": (
                        np.array(wtrace)[: iteration + 1, :] if save_wtrace else None
                    ),
...
if __name__ == "__main__":
    ...
    print(f"scg w is {result['w'][0]:.3f}", result["n_iterations"], result["reason"])
    ...
    steps = 1000
    result = sgd(
        w, rosenbrock, [], steps, rosenbrock_grad, learning_rate=0.001, save_wtrace=True
    )
    print(f"sgd w is {result['w']}")
    path_sgd = result["wtrace"]

    result = adam(
        w, rosenbrock, [], steps, rosenbrock_grad, learning_rate=0.05, save_wtrace=True
    )
