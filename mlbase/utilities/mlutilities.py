...
def regression_summary(T, Y, units='', plot=True, filename=None):
    ...
    if plot:
        ...
        ax.plot(np.arange(mi, ma), np.arange(mi, ma),
...
def partition(X, T, trainFraction, shuffle=False, classification=False, seed=1234):
    ...
    if not classification:
        ...
        for c in classes:
            ...
            if nValidate > 0:
                validateIndices += rowIndices[cRows[nTrain:nTrain +
                                                    nValidate]].tolist()
            testIndices += rowIndices[cRows[nTrain +
                                            nValidate:nTrain+nValidate+nTest]].tolist()
...
def print_confmat(confmat, class_names=None):
    ...
    t = ['{c:<{spacing}} |'.format(c=classes[j], spacing=str(class_spacing-1)) + ''.join(' {i:<{spacing}}'.format(
