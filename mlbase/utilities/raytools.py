...
def ssh_on(host, command):
    if host:
        subprocess.call(
            ["ssh", host, *locale_prep, *command.split(), ">", "/dev/null", "2>&1"]
        )
    else:
...
def ray_start(primary, port, hosts=None, num_gpus=None, pythonpath=None):
    ...
    path = os.getcwd()
    ssh_on(None, f"ray --logging-level=error start --head --port={port}" + gpu_str)
    if hosts:
        for host in hosts:
            ssh_on(
                host,
                f"cd {path} ; hostname ; {pypath} ray --logging-level=error start --address={primary}.cs.colostate.edu:{port}"
                + gpu_str,
            )
    ray_hosts = hosts
    time.sleep(5)
    ...
    ray.init(
        address=f"{primary}.cs.colostate.edu:{port}",
        log_to_driver=True,
...
if __name__ == "__main__":
    ...
    print(f"Finished in: {(time.time() - start_t):0.4f} seconds")
