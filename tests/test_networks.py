...
if __name__ == '__main__':
    ...
    nnet = pynn.NeuralNetwork(X.shape[1], n_hiddens_list,
                              T.shape[1], activation=activation)
    nnet.train(X, T, n_epochs, method=optimizer,
    ...
    nnet = torchnn.NeuralNetwork(X.shape[1], n_hiddens_list,
                                 T.shape[1], activation_f=activation)
    nnet.summary()
    nnet.train(X, T, n_epochs, batch_size=-1,
               learning_rate=learning_rate, opt=optimizer)
    Y = nnet.use(X)
    print(f'RMSE {ml.rmse(Y, T):.3f} in {nnet.training_time:.3f} s')
    ...
    nnet = tfnn.NeuralNetwork(X.shape[1], n_hiddens_list,
                              T.shape[1], activation=activation)

    nnet.train(X, T, n_epochs, batch_size=X.shape[0],
