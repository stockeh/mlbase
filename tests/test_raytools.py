...
def main():
    ...
    def run(hiddens):
        ...
        print(host, hiddens)
        # train network
        nnet = nn.NeuralNetworkClassifier(Xtrain.shape[1:], hiddens, len(
            np.unique(Ttrain)), use_gpu=True, seed=1234)
        nnet.train(Xtrain, Ttrain, n_epochs=100, batch_size=128, learning_rate=0.001,
                   opt='accsgd', validation_data=(Xtest, Ttest), verbose=False)
        # evaluate
        results = [host, hiddens,
                   ml.percent_correct(Ttrain, nnet.use(Xtrain)),
                   ml.percent_correct(Ttest, nnet.use(Xtest))]
        return results, nnet

    start_t = time.time()
    results = ray.get([run.remote([u]*l) for u in [10, 20]
                      for l in range(1, 5)])
